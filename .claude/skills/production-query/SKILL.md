...
never run an unverified binary. In CI, install the CLI with
`scripts/provision-observability-clis.sh` (checksum-verified, fail-closed on a
mismatch or an unverifiable download; falls back to a reviewed `BLUEBOX_CLI_SHA256`
pin). Run provisioning in a step with **no `BLUEBOX_TOKEN` in scope**, and
introduce the token only in the `bluebox ask` step below. This ordering
(provision → integrity-verify → token-only-in-the-ask-step) mirrors the same
discipline Bluebox's own release gates use internally. The checksum-verified
download needs egress to `github.com` +
`release-assets.githubusercontent.com` (dtctl), `app.bluebox.ai` (bluebox CLI +
`ask`), and your Dynatrace tenant host — a blocked host means the CLI is silently
absent and the gate can't run. The authoritative allowlist (with the exact hosts
and why each is needed) is in
[`docs/contributing/claude-code-web-setup.md`](../../../docs/contributing/claude-code-web-setup.md).
...
- Only if `bluebox` is **not found**: provision it with
  `scripts/provision-observability-clis.sh` — the checksum-verifying installer
  (fail-closed on a bad download); it writes to `~/.bluebox/bin` and adds it to
  PATH. The script can't export PATH back to the calling shell, so
  `export PATH="$HOME/.bluebox/bin:$PATH"` and re-check `command -v bluebox`
  afterward. For **interactive** work, an already-resolving `bluebox` is fine as-is;
  don't reorder PATH (an older build may shadow the one you were using).
- **In a CI gate, prove integrity — the provisioner's exit code is not proof.**
  The provisioner is **fail-open**: on a blocked download or missing checksum it
  exits 0 and keeps any pre-existing `~/.bluebox/bin/bluebox`; if that dir is
  absent, PATH can fall through to an ambient, unverified binary. So "run the
  provisioner, then ask" does **not** uphold "never run an unverified binary".
  Assert the resolved binary against your reviewed pin in the **token-free** step
  and fail closed before `BLUEBOX_TOKEN` is ever exposed (same
  "Verify CLI integrity (fail closed)" step as `cut-prod-release`):

  ```sh
  scripts/provision-observability-clis.sh                       # no token in scope
  export PATH="$HOME/.bluebox/bin:$PATH"
  bin="$(command -v bluebox)" || { echo "bluebox not installed"; exit 1; }
  echo "${BLUEBOX_CLI_SHA256:?pin required}  $bin" | sha256sum -c - || exit 1
  ```

  Only after that check passes do you expose `BLUEBOX_TOKEN` for the `ask` step —
  which now provably runs the pinned, verified binary regardless of what was on
  PATH before.
